...
The CLI's `-I`/`--input-mode` vocabulary is broader and is intentionally different from the REST envelope names: `urls` (`url`, `list`), `nuclei-output` (`nuclei`), `openapi` (`swagger`), `wsdl` (`soap`, `svc`), `postman`, `curl`, `burpraw` (`burp-raw`, `raw`), `burpxml` (`burp-xml`, `burp`, `burpstate`), `har` (`http-archive`), and `deparos` (`deparos-output`). Run `vigolium --list-input-mode` for examples generated by the current binary.
