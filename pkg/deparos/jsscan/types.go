...
var (
	...
	ErrIncompatibleProtocol = errors.New("incompatible jsscan protocol")

	// ErrIncompleteOutput indicates the helper did not emit a valid completion record.
	ErrIncompleteOutput = errors.New("incomplete jsscan output")

	// ErrInputTooLarge indicates the configured input budget was exceeded.
	ErrInputTooLarge = errors.New("jsscan input exceeds configured limit")

	// ErrOutputTooLarge indicates the helper exceeded its output budget.
	ErrOutputTooLarge = errors.New("jsscan output exceeds configured limit")
)
...
// Capabilities is returned by `jsscan --capabilities` before any scan runs.
