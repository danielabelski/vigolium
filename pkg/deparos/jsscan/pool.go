...
func (p *WorkerPool) startWorker() (*framedWorker, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create jsscan worker stdin: %w", err)
	}
	stdout, err := cmd.StdoutPipe()
	if err != nil {
		_ = stdin.Close()
		return nil, fmt.Errorf("create jsscan worker stdout: %w", err)
	}
	...
	if err := cmd.Start(); err != nil {
		_ = stdin.Close()
		_ = stdout.Close()
		return nil, fmt.Errorf("start jsscan worker: %w", err)
	}
	...
	case outcome := <-helloCh:
		if outcome.err != nil {
			worker.stop(false)
			return nil, fmt.Errorf("%w: worker hello: %v", ErrIncompleteOutput, outcome.err)
		}
...
func (p *WorkerPool) ScanWithOptions(ctx context.Context, content []byte, options ScanOptions) (*ScanResult, error) {
	...
	for attempt := 0; attempt < 2; attempt++ {
		...
		if replaceErr != nil {
			return nil, fmt.Errorf("%w: %v; replacement: %v", ErrScanFailed, runErr, replaceErr)
		}
		if attempt == 0 {
			p.retries.Add(1)
			continue
...
func (p *WorkerPool) runJob(worker *framedWorker, content []byte, options ScanOptions) (*ScanResult, bool, error) {
	...
	if err != nil {
		return nil, false, fmt.Errorf("create jsscan job directory: %w", err)
	}
	...
	if worker.exited() {
		return nil, false, fmt.Errorf("%w: worker exited: %v; stderr: %s", ErrScanFailed, worker.waitErr, worker.stderr.String())
	}
	if err := writeLengthFrame(worker.stdin, metadata, maxControlFrameBytes); err != nil {
		return nil, false, fmt.Errorf("%w: write worker request: %v", ErrScanFailed, err)
	}
	if err := writeLengthFrame(worker.stdin, content, int64(options.MaxInputBytes)); err != nil {
		return nil, false, fmt.Errorf("%w: write worker content: %v", ErrScanFailed, err)
	}
	payload, err := readLengthFrame(worker.stdout, options.MaxOutputBytes+workerResponseOverhead)
	if err != nil {
		return nil, false, fmt.Errorf("%w: read worker response: %v; stderr: %s", ErrIncompleteOutput, err, worker.stderr.String())
	}
	var response workerResultRecord
	if err := json.Unmarshal(payload, &response); err != nil {
		return nil, false, fmt.Errorf("%w: decode worker response: %v", ErrIncompleteOutput, err)
	}
