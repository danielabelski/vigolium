...
var ErrServiceClosed = errors.New("jsscan service is closed")
...
// in-flight coalescing for every JSSCAN consumer in the process.
type Service struct {
...
func ConfigureDefaultService(config *ServiceConfig) error {
	...
	if defaultService != nil || defaultServiceErr != nil {
		return fmt.Errorf("jsscan default service already initialized")
	}
	copyConfig := *config
...
// passive modules. Scanner remains available as a compatibility low-level API.
...
func cloneScanResult(result *ScanResult) (*ScanResult, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("clone jsscan result: %w", err)
	}
