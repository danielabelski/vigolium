...
// Integration tests that require the actual jstangle binary
...
func TestIntegration_ScanWithRequests(t *testing.T) {
	...
	service := newScanService(t)

	// JavaScript with clear API endpoints that jstangle should detect
	js := `
// API Configuration
...
`
	...
	result, err := service.Scan(context.Background(), []byte(js))
	...
	if len(result.Requests) == 0 {
		t.Log("Warning: no requests extracted - this may be expected depending on jstangle version")
	}
}
...
func TestIntegration_ScanFile_LargeFile(t *testing.T) {
	...
	// Build large JS content with unique variable names to avoid duplicate declaration errors
	var content []byte
...
`)...)
	...
	start := time.Now()
	result, err := service.Scan(context.Background(), content)
