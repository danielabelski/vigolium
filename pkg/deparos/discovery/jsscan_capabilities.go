...
// processJSScanCapabilityFacts connects non-legacy record families to
...
// consumers.
func (e *Engine) processJSScanCapabilityFacts(sourceURL string, result *jsscan.ScanResult) {
	if result == nil {
		return
	}
	if e.storage != nil {
		if repo := e.storage.Extractions(); repo != nil {
			...
			if err := repo.BatchStoreJSScanCapabilityFacts(
				sourceNodeID, e.storage.SessionDBID(), sourceURL, result,
			); err != nil {
				logger.Warn("Failed to store jsscan capability facts", zap.String("source", sourceURL), zap.Error(err))
			}
		}
	}

	generated := make([]jsscan.HTTPRequestFact, 0, len(result.GraphQLOperations))
	...
	if e.config.JSScan.ProtocolHandshake {
	...
	if len(generated) > 0 {
		if parsed := parseURL(sourceURL); parsed != nil {
			e.storeJSScanFacts(parsed, generated)
		}
	}
	...
	for i := range result.ClientRoutes {
		...
		if route.LazyAsset != nil {
			e.processAssetFacts(e.ctx, sourceURL, nil, []jsscan.AssetReferenceFact{{
...
func staticHeader(name, value string) jsscan.HeaderTemplate {
	return jsscan.HeaderTemplate{
		Name:  jsscan.ValueTemplate{Rendered: name, Static: true},
		Value: jsscan.ValueTemplate{Rendered: value, Static: true},
	}
}

func webSocketHandshakeRequest(fact *jsscan.WebSocketFact) (jsscan.HTTPRequestFact, bool) {
	if fact == nil || fact.URL.Rendered == "" {
		return jsscan.HTTPRequestFact{}, false
	}
	...
	return jsscan.HTTPRequestFact{
		Kind: "httpRequest", ID: fact.ID + "-handshake", URL: jsscan.ValueTemplate{Rendered: rawURL, Static: fact.URL.Static, Variables: fact.URL.Variables},
		Method: jsscan.ValueTemplate{Rendered: "GET", Static: true}, Headers: headers, Client: "protocol",
		Provenance: jsscan.Provenance{Extractor: "websocket-handshake", Confidence: "high", Start: fact.Provenance.Start, Evidence: fact.Provenance.Evidence},
	}, true
}

func eventSourceHandshakeRequest(fact *jsscan.EventSourceFact) (jsscan.HTTPRequestFact, bool) {
	if fact == nil || fact.URL.Rendered == "" {
		return jsscan.HTTPRequestFact{}, false
	}
	headers := []jsscan.HeaderTemplate{staticHeader("Accept", "text/event-stream")}
	...
	return jsscan.HTTPRequestFact{
		...
		Method: jsscan.ValueTemplate{Rendered: "GET", Static: true}, Headers: headers, Client: "protocol",
		Provenance: jsscan.Provenance{Extractor: "eventsource-handshake", Confidence: "high", Start: fact.Provenance.Start, Evidence: fact.Provenance.Evidence},
	}, true
}

func graphQLOperationRequest(operation *jsscan.GraphQLOperationFact) (jsscan.HTTPRequestFact, bool) {
	...
	templateVariables := make([]jsscan.TemplateVariable, 0, len(operation.Variables))
	for _, variable := range operation.Variables {
		...
		templateVariables = append(templateVariables, jsscan.TemplateVariable{Name: variable.Name, Placeholder: "${" + variable.Name + "}"})
	}
	if len(variables) > 0 {
	...
	return jsscan.HTTPRequestFact{
		...
		Body: &jsscan.BodyTemplate{
			Kind: "json", ContentType: "application/json",
			Value: jsscan.ValueTemplate{Rendered: string(body), Static: len(templateVariables) == 0, Variables: templateVariables},
		},
		Client: "graphql", Provenance: provenance,
	}, true
}
