...
// TestPrependVerbatimPrompt covers the contract for the task-guidance prompt →
// operator-agent instruction channel. The verbatim prompt (--prompt /
// positional) must come FIRST so the user's exact words (including exploitation
// hints and origin/scope constraints) lead the operator's context; any
// --plan-file / intent-parsed instruction layers on top of that. The three edge
// cases below are the load-bearing ones: both present, only one side present,
// both empty.
func TestPrependVerbatimPrompt(t *testing.T) {
	t.Run("verbatim first, instruction appended", func(t *testing.T) {
		...
		assert.Equal(t,
			...
			got,
			"verbatim prompt must precede the resolved instruction")
	})
...
// multi-app fan-out combines the base instruction with each app's per-app
// instruction. Order: base (--plan-file instruction) first, then app.Instruction.
