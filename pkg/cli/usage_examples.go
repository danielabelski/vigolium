...
var scanExamples = FormatExamples(
	...
	"cat urls.txt | vigolium scan",
	"",
	"# Run specific modules only (fuzzy match on ID/name, active only)",
	"vigolium scan -t https://example.com -m xss-reflected,sqli-error",
	"# Filter modules by tag",
	"vigolium scan -t https://example.com --module-tag spring --module-tag injection",
	"# Run exactly the given module IDs (exact match; unlike -m, also selects passive modules)",
	"vigolium scan -t https://example.com --module-id js-beautify --module-id secret-detect",
	"# Run only passive modules (no active scanning)",
	"vigolium scan -t https://example.com --passive-only",
...
var serverExamples = FormatExamples(
	...
	"# Merge live Burp Proxy history into the normal HTTP records API",
...
var configExamples = FormatExamples(
	...
	"# View settings with a glob/wildcard pattern",
	"vigolium config view 'notify.*'",
	"vigolium config ls '*burp*'",
	"# Enable the Burp bridge (merge live Burp Proxy history) and point it at Burp",
	"vigolium config set server.enable_burp_bridge true",
	"vigolium config set server.burp_bridge_url http://127.0.0.1:9009",
	"# Turn on notifications, pick a channel, and gate by severity",
...
var configSetExamples = FormatExamples(
	...
	"vigolium config set notify.enabled true",
	"# Pick a single notification channel (webhook|telegram|discord)",
	"vigolium config set notify.provider telegram",
	...
	"vigolium config set notify.telegram.bot_token <bot-token>",
	"vigolium config set notify.telegram.chat_id <chat-id>",
	"# Enable the Burp bridge and set its loopback address",
	"vigolium config set server.enable_burp_bridge true",
	"vigolium config set server.burp_bridge_url http://127.0.0.1:9009",
...
var trafficExamples = FormatExamples(
	...
	"vigolium traffic",
	"# Merge live Burp Proxy history with records in the local database",
	"vigolium traffic --burp-bridge-url http://127.0.0.1:9009",
	"# Show only the live Burp portion of the merged traffic",
	"vigolium traffic --burp-bridge-url http://127.0.0.1:9009 --source burp",
	"# ...or --source caido when the listener is the Caido plugin",
	"vigolium traffic --caido-bridge-url http://127.0.0.1:9009 --source caido",
	"# Persist the displayed Burp page into the database, then show the merged view",
	"vigolium traffic --burp-bridge-url http://127.0.0.1:9009 --save-to-vigolium-db",
	"# Import every matching live Burp record (filters such as --host still apply)",
	"vigolium traffic --burp-bridge-url http://127.0.0.1:9009 --save-to-vigolium-db --all",
	"# Copy the displayed database page into Burp's Target Site map",
	"vigolium traffic --burp-bridge-url http://127.0.0.1:9009 --save-to-burp",
	"# Copy every matching database record to the Site map",
	"vigolium traffic --burp-bridge-url http://127.0.0.1:9009 --save-to-burp --all",
	...
	"# --search spans URL, path, and the raw request/response (headers + body); repeat to AND-narrow",
	`vigolium traffic --search "/api/users"`,
	`vigolium traffic --search "api" --search "token"`,
	...
	"# Exclude (inverse of --search/--header/--body); drop rows where the term appears. Repeat --exclude-search to drop more",
	`vigolium traffic --search "api" --exclude-search "/health"`,
	`vigolium traffic --exclude-search ".css" --exclude-search ".js"`,
	`vigolium traffic --exclude-body "heartbeat"`,
	"",
	"# --- Filter ---",
	...
	"",
	"# --- Stateless (read exports without touching your project DB) ---",
	"# Browse a standalone .sqlite / .jsonl export as a tree (project scoping off)",
	"vigolium traffic -S --db scan.sqlite --tree",
	"# Merge a glob of result files into one temporary DB, then browse them (--glob-db implies -S)",
	"vigolium traffic -S --glob-db 'roc-*.sqlite' --tree",
	...
	"vigolium traffic -S --db scan.sqlite --replay --all --proxy http://127.0.0.1:8080 -c 5",
	"# Replay only the POSTs to one host through Burp",
	"vigolium traffic --replay --all --method POST --host api.example.com --proxy http://127.0.0.1:8080",
	...
	"vigolium traffic --replay --all --with-browser --proxy http://127.0.0.1:8080",
	"# Replay and overwrite stored responses",
	"vigolium traffic --replay --host example.com --in-replace",
	"",
	"# --- Send a stored request to Burp's Repeater / Organizer ---",
	"# traffic locates the record; grab its full UUID (TUI 'c' to copy, or -j '.records[].uuid'),",
	"# then hand it to replay --to-repeater / --to-organizer (needs --burp-bridge-url, not --proxy):",
	`vigolium traffic --host api.example.com --status 200 -j | jq -r '.records[].uuid'`,
	"# Stage that request in a Burp Repeater tab for manual testing",
	"vigolium replay -u <record-uuid> --to-repeater -B http://127.0.0.1:9009",
	"# ...into a named Repeater tab",
	"vigolium replay -u <record-uuid> --to-repeater --repeater-tab idor -B http://127.0.0.1:9009",
	"# Store the request + response in Burp's Organizer with a note and highlight colour",
	`vigolium replay -u <record-uuid> --to-organizer --notes "idor candidate" --highlight orange -B http://127.0.0.1:9009`,
	"# Re-issue byte-for-byte through Burp's own engine as it stages in Repeater",
	"vigolium replay -u <record-uuid> --to-repeater --send-via-burp -B http://127.0.0.1:9009",
	"# Bulk alternative: copy every matched record into Burp's Target Site map",
	"vigolium traffic --host api.example.com --save-to-burp -B http://127.0.0.1:9009 --all",
)
...
var exportExamples = FormatExamples(
	...
	"# Export from a custom / standalone database file",
	"vigolium export --db custom.sqlite",
	"# Export to a flat, browsable filesystem tree (request/response + finding files)",
	"vigolium export --db custom.sqlite --format fs",
...
var findingExamples = FormatExamples(
	...
	"# --search spans module metadata, matched location, and the linked request/response",
	"vigolium finding --search 'Reverse Proxy Path'",
	"# Repeat --search to narrow — every term must match (AND), in any order",
	"vigolium finding --search 'Reverse Proxy' --search 'metrics'",
	"# Exclude (inverse of --search); drop findings where the term appears. Repeatable",
	"vigolium finding --exclude-search nuclei",
	"vigolium finding --min-severity high --exclude-search 'self-signed'",
	"# Read a whole directory of standalone scan DBs merged into one (implies -S)",
	"vigolium finding -S --glob-db 'scans/*.sqlite' --min-severity high",
	"# glob-db + multi-search, rendered as Markdown (response bodies compacted)",
	"vigolium finding -S --glob-db 'scans/*.sqlite' --search 'jwt' --search 'alg' --markdown",
	...
	"# Render matches as Markdown (colored on a TTY, plain when piped/redirected)",
	"vigolium finding xss --markdown",
	"# Markdown with whole response bodies (default compacts them to a preview)",
	"vigolium finding xss --markdown --full-body > findings.md",
	"# JSONL output",
	"vigolium finding -j",
	"# JSON for a coding agent, with linked HTTP records embedded",
	"vigolium finding --min-severity medium --json --with-records",
	"# TRIAGE: push a finding's evidence to Burp's Organizer for manual confirmation",
	"vigolium finding --id 42 --push-to-burp --burp-bridge-url http://127.0.0.1:9009",
	"# TRIAGE: re-issue high+ findings through Burp's engine, storing fresh responses",
	"vigolium finding --min-severity high --push-to-burp --send-via-burp \\",
	"  --burp-bridge-url http://127.0.0.1:9009",
	"# TRIAGE: open one finding's request in a Burp Repeater tab for hand testing",
	"vigolium finding --id 42 --to-repeater --burp-bridge-url http://127.0.0.1:9009",
...
var agentAutopilotExamples = FormatExamples(
	...
	"# Source-aware scan with specific files and custom guidance",
	`vigolium agent autopilot -t https://example.com --source ./src --files "routes/api.js,controllers/auth.js" --prompt "Focus on the new payment endpoint"`,
...
var agentSwarmExamples = FormatExamples(
	...
	"# Add custom guidance to steer the agent",
	`vigolium agent swarm --input "https://example.com" --prompt "Focus on auth bypass and IDOR"`,
	"# Load a whole plan (prose + raw HTTP request seeds) from a file",
	`vigolium agent swarm -t https://example.com --plan-file ./pentest-notes.md`,
...
var scanURLExamples = FormatExamples(
	...
	"# Run only specific modules (fuzzy match on ID/name, active only)",
	"vigolium scan-url -m sqli -m xss https://example.com/search?q=test",
	"# Run exactly one module by ID (exact match; also selects passive modules, e.g. the JS beautifier)",
	"vigolium scan-url --module-id js-beautify https://example.com/app.js",
	"# Skip passive analysis (active modules only)",
	"vigolium scan-url --no-passive https://example.com/api/users",
	"# Run only passive modules (no active scanning)",
	"vigolium scan-url --passive-only https://example.com/api/users",
...
var scanRequestExamples = FormatExamples(
	...
	"# Scan with specific modules only (fuzzy match on ID/name, active only)",
	`printf 'GET /search?q=test HTTP/1.1\r\nHost: example.com\r\n\r\n' | vigolium scan-request -m sqli -m xss`,
	"# Run exactly one module by ID (exact match; also selects passive modules)",
	"vigolium scan-request -i request.txt --module-id secret-detect",
	"# Skip passive modules (active modules only)",
	"vigolium scan-request -i request.txt --no-passive",
	"# Run only passive modules (no active scanning)",
	"vigolium scan-request -i request.txt --passive-only",
...
var importExamples = FormatExamples(
	"# Import all bridge-visible Burp Proxy history into the configured database",
	"vigolium import --burp-bridge-url http://127.0.0.1:9009",
	"# Import live Burp traffic into an explicit database/project",
	"vigolium import --burp-bridge-url http://127.0.0.1:9009 --db ./project.sqlite --project-name my-project",
	"",
	...
	"# Merge several scan DBs at once into one destination",
	"vigolium import --db combined.sqlite scan-a.sqlite scan-b.sqlite scan-c.sqlite",
	"# Merge every matching DB via a glob (idempotent — re-runs are no-ops)",
	"vigolium import --db combined.sqlite --glob-db 'scans/*.sqlite'",
	"# Same idea for JSONL exports — pass several, or glob them (use one format per run)",
...
var fuzzExamples = FormatExamples(
	...
	"",
	"# BURP ENGINE: send every payload through Burp byte-for-byte (force HTTP/1 for smuggling)",
	"vigolium fuzz -i req.txt --point PATH:0 -w smuggle.txt \\",
	"  --send-via-burp --http-mode http1 --burp-bridge-url http://127.0.0.1:9009",
	"# BURP: fuzz through Burp's engine and push matches to Organizer for triage",
	"vigolium fuzz 'https://acme.test/item?id=FUZZ' --class sqli --match-status-code 500 \\",
	"  --send-via-burp --matches-to-organizer --burp-bridge-url http://127.0.0.1:9009",
	"",
...
var replayExamples = FormatExamples(
	"# Re-send a stored record and diff baseline vs replay (e.g. under fresh cookies)",
	"vigolium replay --record-uuid abc12345",
	"# Re-send a curl command verbatim (auto-baseline by re-sending)",
	`vigolium replay -i "curl -X POST https://example.com/api/login -d 'u=admin'"`,
	"# Re-confirm a finding's stored evidence",
	"vigolium replay --finding-id 42",
	"# Send exact bytes verbatim (raw override) — e.g. re-send a captured request",
	"vigolium replay -i req.txt --raw-request-file exact.txt",
	"# NOTE: for payload / insertion-point fuzzing, use `vigolium fuzz` instead:",
	"#   vigolium fuzz -u abc12345 --point URL_PARAM:id --class sqli",
	"",
	"# Add the request and fresh response directly to Burp's Site map",
	"vigolium replay --record-uuid abc12345 \\",
	"  --save-to-burp --burp-bridge-url http://127.0.0.1:9009",
	"# BURP ENGINE: send exact bytes byte-for-byte through Burp (force HTTP/1 for desync)",
	"vigolium replay -i req.txt --raw-request-file smuggle.txt \\",
	"  --send-via-burp --http-mode http1 --burp-bridge-url http://127.0.0.1:9009",
	"# Confirm a finding via Burp's engine and stash the exchange in Organizer",
	"vigolium replay --finding-id 42 --send-via-burp --to-organizer \\",
	"  --notes 'confirm #42' --highlight red --burp-bridge-url http://127.0.0.1:9009",
	"# Stage a request in a Burp Repeater tab for hand testing",
	"vigolium replay --record-uuid abc12345 \\",
	"  --to-repeater --repeater-tab idor-1 --burp-bridge-url http://127.0.0.1:9009",
	"",
	...
	"# Re-send against a different environment than the baseline",
	...
	"# BULK: search stored traffic (fuzzy, like `vigolium traffic admin`) and replay the matches",
	"vigolium replay admin --proxy http://127.0.0.1:8080 -c 5",
	"# BULK: pattern-search like `vigolium traffic` — POSTs to an API host, drop logout",
	"vigolium replay --host api.example.com --method POST --search token \\",
	"  --exclude-search logout --proxy http://127.0.0.1:8080 -c 5",
	"# BULK: replay 2xx JSON responses seen today, most recent first",
	"vigolium replay --status 200 --body 'application/json' --from 2026-07-21 \\",
	"  --sort sent_at -c 5",
	"# BULK: replay and add each fresh exchange to Burp's Target Site map",
