...
function ProtocolRenderer({ value }: { value: string }) {
  const color = PROTOCOL_COLORS[value?.toLowerCase()] || '#918175';
  return (
        ...
        <div className={`border border-[#2e2b26] bg-[#1c1b19] overflow-hidden flex flex-col ${selectedId !== null ? 'w-1/2' : 'w-full'} transition-all`}>
          <div className="px-3 py-1.5 border-b border-[#2e2b26] flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-1.5">
              <span className="text-[#7fd962] text-xs font-bold">OAST_INTERACTIONS</span>
          ...
          <div className={`${AG_GRID_THEME} w-full flex-1`}>
            {isLoading && !data ? (
              ...
              <AgGridReact<OASTInteraction>
                ...
                overlayNoRowsTemplate='<span style="color:#403d38">no interactions</span>'
              />
            )}
          </div>
