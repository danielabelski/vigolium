          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              ...
              {!scansLoading && (!scansData?.data || scansData.data.length === 0) && (
                <tr>
                  <td colSpan={7} className="px-3 py-4 text-center text-[#403d38] v-fade-in">no scans</td>
                </tr>
              )}
              {scansData?.data?.map((scan) => (
                <tr
                  key={scan.uuid}
                  className="border-b border-[#2e2b26]/50 hover:bg-[#272520] transition-colors v-fade-in"
                >
