...
export async function jsscan(
  ...
  const stageRuns: Record<StageName, () => void | Promise<void>> = {
    ...
    // a `+` chain that merge-strings then folds).
    deobfuscate: () => {
      if (ast) {
        applyTransformsToFixpoint(ast, [concatToPlus, mergeStrings, controlFlowObject], {
  ...
  const stages: Stage[] = buildStagePlan(context.capabilities).map((planned) => ({
    ...planned,
    enabled: planned.enabled && (planned.name !== 'beautify' || looksWorthBeautifying(code)),
  ...
  if (context.has('endpoints') && !context.failed) {
    ...
    for (const [key, values] of Object.entries(trackedVars)) {
      if (values && values.length > 0) trackedVarsSimple[key] = values[0];
    }
    for (const request of getWebpackExtractedRequests(trackedVarsSimple)) {
